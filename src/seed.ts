  ...
  try {
    ...
    for (const id of userIds) {
      ...
      if (!existing) {
        ...
        user.rights = ROLE_ADMIN;

        await (usersRepo as any).repository.save(user);
      }
    }
    ...
    for (const groupData of mockGroupsData) {
      ...
      const adminId =
        groupData.members.find((m) => m.role === 'admin')?.userId || userIds[0];
      ...
      for (const habit of groupData.habits) {
        const h = await groupsService.addHabit(
          createdGroup.id,
          adminId as string,
          {
            name: habit.name,
